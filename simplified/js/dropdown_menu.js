/****
*/
;(function($) { $.fn.DropdownMenu = function ( config ) {
  if ( ! config ) { config = {}; }
  ...
  var helper = {
    ...
    **************************************************************************/
    init : function() {
      ...
      opt.counter = $('<div class="counter"></div>').appendTo(opt.main);
      ...
      opt.btn     = opt.main.find('>.dropdown_btn');
      opt.list    = opt.main.find('.dropdown_list');
      opt.option  = opt.main.find('.dropdown_option').each(function(i,d){
      ...
      var prev = opt.input.prev('.dropdown_btn');
    ...
    initSearchField : function() {
      var node = $(
        '<div class="dropdown_menu_search_field_wrapper">' +
          '<input type="text" class="dropdown_menu_search_field" value="">' +
        '</div>'
      ).appendTo( opt.main.addClass('included_search_field') ); 

      opt.field = node.find('.dropdown_menu_search_field');
    ...
    initList : function( value ) {
      ...
      opt.main.find('.dropdown_list').remove();
      opt.list = $(
        '<ul role="listbox" aria-expanded="false" class="dropdown_list"></ul>'
      ).appendTo( opt.main );
      var id = helper._generateId( opt.list );
      opt.main.find('>.dropdown_btn').attr('aria-controls', id);
      ...
      if ( opt.multiple ) {
        opt.tool = $(
          '<div class="dropdown_multiple_tool">'+
        ...
        if ( opt.toolWrapper && opt.toolWrapper.size() ) {
          ...
          opt.listWrapper = $('<div class="dropdown_list_wrapper"></div>')
    ...
    **************************************************************************/
    ...
    appendOption : function( startIndex, reset, previous ) {
      ...
      var box = opt.list || opt.main.find('>.dropdown_list');
      if ( ! box.size() ) { return; }
      ...
      if ( startIndex === -1 && helper.isSelectedIndexEmpty() ) {
        temp.push( 
          '<li class="dropdown_option hide" role="option" data-value="" tabindex="-1">'+
            opt.defaultLabel +
          '</li>'
        ); 
        startIndex = 0;
      ...
      if ( loop ) {
        ...
        for ( i; i<j; i++ ) {
          ...
          var type  = 'dropdown_option' + (test ? ' selected' : '');
        ...
        if ( horizontal && temp.length<opt.count ) {
          ...
          for ( var k=0; k<diff; k++ ) {
            temp.push(
              '<li class="dropdown_empty hidden" aria-hidden="true" tabindex="-1">'+
      ...
      if ( ! temp.length && !s ) { 
        temp.push('<li class="dropdown_empty" aria-hidden="true">Ingen treff</li>');
      } else if ( (loop > i && opt.expander) || horizontal ) {
        var rest = loop-i; 
        var next = opt.count > rest ? rest : opt.count;

        var menu = horizontal ? 'dropdown_expander' : 'dropdown_option';
    ...
    reset : function () {
      opt.main.find('.dropdown_option').eq(0).click();
    },
    ...
    changeOptionFocus : function( up, force ) {
      var index = null, slc = '.hide,.dropdown_empty' +
    ...
    toggleAllMultipleSection : function( force ) {
      ...
      if ( force ) {
        ...
        opt.list.find('.dropdown_option').addClass( mode );
      } 
      else {
        opt.selectedIndex = {};
        opt.list.find('.dropdown_option').removeClass( mode );
      }
    ...
    **************************************************************************/
    ...
    _changeAriaHidden: function( node ) {
      ...
      /*
      opt.btn.find('>.shy_info').remove();
      var holder = opt.btn.find('>.info_holder');
      if ( ! holder.size() && node && node.size() ) { 
        holder = opt.btn.html( 
          '<div aria-hidden="true" class="info_holder">'+opt.btn.html()+'</div>' 
        );
      } 
      ...
      if ( node && node.size() ) {
        var a = node.find('a');
        opt.btn.append(
          '<div class="shy_info" style="visibility:hidden;position:absolute;left:-100;top:0">'+
    ...
    _displayCountView : function( sum ) {
      ...
      var view = opt.list.children().not('.hide,.option_expander,.dropdown_empty').size();
    ...
    _click : function( e ) {
      ...
      var exp = 'option_expander', btn = 'dropdown_btn';
      e.preventDefault();
      ...
      if ( node.hasClass('dd_multiple_all_toggler') ) {
        ...
        helper.setSelected( node.closest('.dropdown_option') );
        if ( opt.multiple ) { 
          clearTimeout( opt.timer.hideList || 0 );
    ...
    _mousemove : function( e ) {
      ...
      $(e.target).closest('.dropdown_option').addClass('focus');
    },
